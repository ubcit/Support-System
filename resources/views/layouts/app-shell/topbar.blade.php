<header
    class="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-gray-200 bg-shell-canvas px-4 dark:border-gray-800 sm:px-6"
...
>
    ...
    <button
        ...
        class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-white/5 lg:hidden"
    ...
    {{-- Breadcrumb --}}
    <div class="flex min-w-0 flex-1 items-center gap-4 overflow-x-auto no-scrollbar">
        <nav aria-label="Breadcrumb" class="shrink-0">
            <ol class="flex items-center gap-1.5 text-sm">
                <li>
                    <a href="{{ $homeDestination['item']['path'] ?? '/' }}" wire:navigate class="text-gray-500 hover:text-brand-500 dark:text-gray-400">
                        {{ $homeDestination['label'] ?? 'Home' }}
                    </a>
                </li>
                @if (($activeDestination['key'] ?? null) !== ($homeDestination['key'] ?? null))
                    <li class="text-gray-300 dark:text-gray-600">/</li>
                    <li class="font-semibold text-gray-800 dark:text-white/90">{{ $activeDestination['label'] ?? '' }}</li>
                @endif
            </ol>
        </nav>
    </div>
    ...
    >
        ...
        <span class="flex h-9 w-full items-center truncate whitespace-nowrap rounded-lg border border-gray-200 bg-transparent py-0 pl-9 pr-14 text-left text-sm leading-none text-gray-400 dark:border-gray-800 dark:bg-white/3 dark:text-white/30">
            Search or type a command...
        </span>
        <span class="absolute right-2 top-1/2 inline-flex shrink-0 -translate-y-1/2 items-center gap-0.5 rounded-lg border border-gray-200 bg-gray-50 px-[7px] py-[3px] text-[11px] leading-none text-gray-500 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400">
    ...
    <div class="flex shrink-0 items-center gap-2">
        ...
        <button
            type="button"
            class="relative flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 text-gray-500 hover:bg-gray-100 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-white/5"
