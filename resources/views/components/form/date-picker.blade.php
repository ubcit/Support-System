...
<div
    ...
    x-data="{
        ...
        isInstanceAlive() {
            ...
            return this.flatpickrInstance.input === input && input.isConnected;
        },
        ensureInstance() {
            if (this.isInstanceAlive()) return true;
            this.mount();
        ...
        mount() {
            ...
            if (this.isInstanceAlive()) return;

            // wire:ignore can leave an orphaned flatpickr on the input after Alpine remounts.
            if (input._flatpickr && input._flatpickr !== this.flatpickrInstance) {
...
>
    ...
    <div class="relative">
        {{-- Keep flatpickr's injected static calendar across Livewire morphs --}}
        <div class="custom-datepicker" wire:ignore>
            <input
                x-ref="dateInput"
                type="text"
                id="{{ $id }}"
        ...
        @if($allowClear)
            <button
                ...
                x-cloak
                @click.prevent="clear()"
                class="absolute right-9 top-1/2 -translate-y-1/2 p-0.5 rounded text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 z-10"
                title="Clear date"
