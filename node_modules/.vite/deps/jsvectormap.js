//#region node_modules/jsvectormap/dist/jsvectormap.esm.js
/**
...
var getElement = function getElement(selector) {
	if (typeof selector === "object" && typeof selector.nodeType !== "undefined") return selector;
...
var createElement = function createElement(type, classes, content, html) {
	...
	if (content) el[!html ? "textContent" : "innerHTML"] = content;
	...
	delegate: function delegate(element, event, selector, handler) {
		...
		event.forEach(function(eventName) {
			EventHandler.on(element, eventName, function(e) {
				var target = e.target;
				if (target.matches(selector)) handler.call(target, e);
			});
		});
	},
...
function setupContainerEvents() {
	...
	if (this.params.zoomOnScroll) EventHandler.on(this.container, "wheel", function(event) {
		...
		if (map.tooltip) map._tooltip.hide();
...
var parseEvent = function parseEvent(map, selector, isTooltip) {
	...
	if (isTooltip) event = isRegion ? Events.onRegionTooltipShow : Events.onMarkerTooltipShow;
	return {
		type,
		code,
		event,
...
function setupElementEvents() {
	...
	EventHandler.on(container, "mousemove", function(event) {
		if (Math.abs(pageX - event.pageX) + Math.abs(pageY - event.pageY) > 2) mouseMoved = true;
	});
	...
	EventHandler.delegate(container, "mouseover mouseout", ".jvm-element", function(event) {
		...
		if (event.type === "mouseover") {
			...
			if (showTooltip) {
				...
				if (!event.defaultPrevented) map._tooltip.show();
			}
		} else {
			data.element.hover(false);
			if (showTooltip) map._tooltip.hide();
		}
	});
	EventHandler.delegate(container, "mouseup", ".jvm-element", function(event) {
		...
		if (data.type === "region" && map.params.regionsSelectable || data.type === "marker" && map.params.markersSelectable) {
			...
			if (map.params[data.type + "sSelectableOne"]) data.type === "region" ? map.clearSelectedRegions() : map.clearSelectedMarkers();
			if (data.element.isSelected) element.select(false);
			else element.select(true);
			map._emit(data.event, [
				data.code,
				element.isSelected,
				data.type === "region" ? map.getSelectedRegions() : map.getSelectedMarkers()
			]);
		}
	});
...
function setupZoomButtons() {
	...
	if (!zoomInOption) this.container.appendChild(zoomIn);
	if (!zoomOutOption) this.container.appendChild(zoomOut);
...
function setupContainerTouchEvents() {
	...
	var handleTouchEvent = function handleTouchEvent(e) {
		...
		if (e.type == "touchstart") lastTouchesLength = 0;
		if (touches.length == 1) {
			if (lastTouchesLength == 1) {
				...
				map._applyTransform();
				if (transXOld != map.transX || transYOld != map.transY) e.preventDefault();
			}
			touchX = touches[0].pageX;
			touchY = touches[0].pageY;
		} else if (touches.length == 2) if (lastTouchesLength == 2) {
			var _map$_tooltip2;
			...
			if (touches[0].pageX > touches[1].pageX) centerTouchX = touches[1].pageX + (touches[0].pageX - touches[1].pageX) / 2;
			else centerTouchX = touches[0].pageX + (touches[1].pageX - touches[0].pageX) / 2;
			if (touches[0].pageY > touches[1].pageY) centerTouchY = touches[1].pageY + (touches[0].pageY - touches[1].pageY) / 2;
			else centerTouchY = touches[0].pageY + (touches[1].pageY - touches[0].pageY) / 2;
...
var BaseComponent = /*#__PURE__*/ function() {
	...
	_proto.dispose = function dispose() {
		if (this._tooltip) removeElement(this._tooltip);
...
var Interactable = {
	getLabelText: function getLabelText(key, label) {
		...
		if (typeof label.render === "function") {
			var params = [];
			if (this.constructor.Name === "marker") params.push(this.getConfig());
			params.push(key);
	...
	getLabelOffsets: function getLabelOffsets(key, label) {
		if (typeof label.offsets === "function") return label.offsets(key);
...
var Line = /*#__PURE__*/ function(_BaseComponent) {
	function Line(options, style) {
		var _this = _BaseComponent.call(this) || this;
		_this._options = options;
		_this._style = { initial: style };
		_this._draw();
		return _this;
	}
	...
	_proto._draw = function _draw() {
		var _this$_options = this._options, index = _this$_options.index, group = _this$_options.group, map = _this$_options.map;
		var config = {
...
function createLines(lines) {
	...
	for (var index in lines) {
		...
		for (var _i = 0, _Object$values = Object.values(this._markers); _i < _Object$values.length; _i++) {
			...
			if (markerConfig.name === lineConfig.from) point1 = this.getMarkerPosition(markerConfig);
...
var Marker = /*#__PURE__*/ function(_BaseComponent) {
	function Marker(options, style) {
		...
		_this._draw();
		if (_this._options.label) _this._drawLabel();
		if (_this._isImage) _this.updateLabelPosition();
		return _this;
	}
	...
	_proto._draw = function _draw() {
		var _this$_options = this._options, index = _this$_options.index, map = _this$_options.map, group = _this$_options.group, cx = _this$_options.cx, cy = _this$_options.cy;
	...
	_proto._drawLabel = function _drawLabel() {
		var _this$_options2 = this._options, index = _this$_options2.index, map = _this$_options2.map, label = _this$_options2.label, labelsGroup = _this$_options2.labelsGroup, cx = _this$_options2.cx, cy = _this$_options2.cy, config = _this$_options2.config, isRecentlyCreated = _this$_options2.isRecentlyCreated;
		...
		if (isRecentlyCreated) this.updateLabelPosition();
	};
...
function createMarkers(markers, isRecentlyCreated) {
	...
	var _loop = function _loop() {
		...
		if (_this._markers[index]) _this.removeMarkers([index]);
...
var Legend = /*#__PURE__*/ function() {
	function Legend(options) {
		...
		if (this._options.cssClass) this._body.setAttribute("class", this._options.cssClass);
		if (options.vertical) this._map.legendVertical.appendChild(this._body);
		else this._map.legendHorizontal.appendChild(this._body);
		this.render();
	}
	...
	_proto.render = function render() {
		...
		for (var i = 0; i < ticks.length; i++) {
			...
			switch (this._series.config.attribute) {
				case "fill":
					if (isImageUrl(ticks[i].value)) sample.style.background = "url(" + ticks[i].value + ")";
					else sample.style.background = ticks[i].value;
					break;
				case "stroke":
			...
			if (this._options.labelRender) label = this._options.labelRender(label);
...
var OrdinalScale = /*#__PURE__*/ function() {
	...
	_proto.getTicks = function getTicks() {
		var ticks = [];
		for (var key in this._scale) ticks.push({
			label: key,
...
var Series = /*#__PURE__*/ function() {
	function Series(config, elements, map) {
		...
		if (config.attributes) this.setAttributes(config.attributes);
		if (typeof config.scale === "object") this.scale = new OrdinalScale(config.scale);
		if (this.config.legend) this.legend = new Legend(merge({
			map: this._map,
			series: this
		}, this.config.legend));
	...
	_proto.setValues = function setValues(values) {
		var attrs = {};
		for (var key in values) if (values[key]) attrs[key] = this.scale.getValue(values[key]);
		this.setAttributes(attrs);
	};
	_proto.setAttributes = function setAttributes(attrs) {
		for (var code in attrs) if (this._elements[code]) this._elements[code].element.setStyle(this.config.attribute, attrs[code]);
	};
	_proto.clear = function clear() {
		var key, attrs = {};
		for (key in this._values) if (this._elements[key]) attrs[key] = this._elements[key].element.shape.style.initial[this.config.attribute];
		this.setAttributes(attrs);
		this._values = {};
	};
	return Series;
...
function createSeries() {
	...
	for (var key in this.params.series) for (var i = 0; i < this.params.series[key].length; i++) this.series[key][i] = new Series(this.params.series[key][i], key === "markers" ? this._markers : this.regions, this);
}
function applyTransform() {
	...
	if (this._markers) this._repositionMarkers();
	if (this._lines) this._repositionLines();
...
function setScale(scale, anchorX, anchorY, isCentered, animate) {
	...
	if (scale > this.params.zoomMax * this._baseScale) scale = this.params.zoomMax * this._baseScale;
	else if (scale < this.params.zoomMin * this._baseScale) scale = this.params.zoomMin * this._baseScale;
...
function setFocus(config) {
	...
	if (config === void 0) config = {};
	var bbox, codes = [];
	if (config.region) codes.push(config.region);
	else if (config.regions) codes = config.regions;
	if (codes.length) {
		codes.forEach(function(code) {
			if (_this.regions[code]) {
				var itemBbox = _this.regions[code].element.shape.getBBox();
				if (itemBbox) if (typeof bbox == "undefined") bbox = itemBbox;
				else bbox = {
...
function getInsetForPoint(x, y) {
	...
	for (var index = 0; index < insets.length; index++) {
		...
		if (x > start.x && x < end.x && y > start.y && y < end.y) return insets[index];
	}
}
function getMarkerPosition(_ref) {
	var coords = _ref.coords;
	if (Map.maps[this.params.map].projection) return this.coordsToPoint.apply(this, coords);
	return {
...
function repositionLines() {
	...
	Object.values(this._lines).forEach(function(line) {
		var startMarker = Object.values(_this._markers).find(function(_ref) {
			return _ref.config.name === line.getConfig().from;
		});
		var endMarker = Object.values(_this._markers).find(function(_ref2) {
			return _ref2.config.name === line.getConfig().to;
		});
		if (startMarker && endMarker) {
			...
			line.setStyle({ d: "M" + x1 + "," + y1 + " Q" + curveX + "," + curveY + " " + x2 + "," + y2 });
		}
	});
}
function repositionMarkers() {
	for (var index in this._markers) {
		...
		if (point !== false) this._markers[index].element.setStyle({
			cx: point.x,
			cy: point.y
		});
	}
...
var SVGElement = /*#__PURE__*/ function() {
	...
	_proto.set = function set(property, value) {
		if (typeof property === "object") for (var attr in property) this.applyAttr(attr, property[attr]);
		else this.applyAttr(property, value);
	};
...
var SVGShapeElement = /*#__PURE__*/ function(_SVGElement) {
	...
	_proto.setStyle = function setStyle(property, value) {
		if (typeof property === "object") merge(this.style.current, property);
		else {
			var _merge;
	...
	_proto.updateStyle = function updateStyle() {
		...
		if (this.isHovered) merge(attrs, this.style.hover);
		if (this.isSelected) {
			merge(attrs, this.style.selected);
			if (this.isHovered) merge(attrs, this.style.selectedHover);
		}
		this.set(attrs);
	};
...
var SVGImageElement = /*#__PURE__*/ function(_SVGShapeElement) {
	...
	_proto.applyAttr = function applyAttr(attr, value) {
		...
		if (attr === "image") {
			...
			this.cx = value;
			if (this.width) this.applyAttr("x", value - this.width / 2 + this.offset[0]);
		} else if (attr == "cy") {
			this.cy = value;
			if (this.height) this.applyAttr("y", value - this.height / 2 + this.offset[1]);
		} else _SVGShapeElement.prototype.applyAttr.apply(this, arguments);
	};
	return SVGImageElement;
}(SVGShapeElement);
var SVGCanvasElement = /*#__PURE__*/ function(_SVGElement) {
	function SVGCanvasElement(container) {
		var _this = _SVGElement.call(this, "svg") || this;
		_this._container = container;
		_this._defsElement = new SVGElement("defs");
		_this._rootElement = new SVGElement("g", { id: "jvm-regions-group" });
...
var Tooltip = /*#__PURE__*/ function(_BaseComponent) {
	...
	_proto._bindEventListeners = function _bindEventListeners() {
		...
		EventHandler.on(this._map.container, "mousemove", function(event) {
			if (!_this2._tooltip.classList.contains("active")) return;
	...
	_proto.text = function text(string, html) {
		...
		if (!string) return this._tooltip[property];
		this._tooltip[property] = string;
	};
	_proto.css = function css(_css) {
		for (var style in _css) this._tooltip.style[style] = _css[style];
		return this;
	};
	return Tooltip;
}(BaseComponent);
var DataVisualization = /*#__PURE__*/ function() {
	...
	_proto.visualize = function visualize() {
		...
		for (var regionCode in this._values) {
			...
			if (!isNaN(value)) attrs[regionCode] = this.getValue(value);
		}
		this.setAttributes(attrs);
	};
	_proto.setAttributes = function setAttributes(attrs) {
		for (var code in attrs) if (this._map.regions[code]) this._map.regions[code].element.setStyle("fill", attrs[code]);
	};
	_proto.getValue = function getValue(value) {
...
var Map = /*#__PURE__*/ function() {
	function Map(options) {
		...
		if (!Map.maps[this.params.map]) throw new Error("Attempt to use map which was not loaded: " + options.map);
		this.regions = {};
		this.scale = 1;
		this.transX = 0;
		this.transY = 0;
	...
	_proto._init = function _init() {
		...
		this._createRegions();
		this.updateSize();
		if (options.lines) this._linesGroup = this.canvas.createGroup(LINES_GROUP_ID);
		if (options.markers) {
		...
		if (options.zoomButtons) this._setupZoomButtons();
		if (options.showTooltip) this._tooltip = new Tooltip(this);
		if (options.selectedRegions) this._setSelected("regions", options.selectedRegions);
		if (options.selectedMarkers) this._setSelected("_markers", options.selectedMarkers);
		if (options.focusOn) this.setFocus(options.focusOn);
		if (options.visualizeData) this.dataVisualization = new DataVisualization(options.visualizeData, this);
		if (options.bindTouchEvents) {
			if ("ontouchstart" in window || window.DocumentTouch && document instanceof DocumentTouch) this._setupContainerTouchEvents();
		}
		if (options.series) {
	...
	_proto.clearSelectedRegions = function clearSelectedRegions(regions) {
		var _this2 = this;
		if (regions === void 0) regions = void 0;
	...
	_proto.removeMarkers = function removeMarkers(markers) {
		var _this3 = this;
		if (!markers) markers = Object.keys(this._markers);
	...
	_proto.removeLines = function removeLines(lines) {
		var _this4 = this;
		if (Array.isArray(lines)) lines = lines.map(function(line) {
			return getLineUid(line.from, line.to);
		});
		else lines = this._getLinesAsUids();
	...
	_proto.reset = function reset() {
		for (var key in this.series) for (var i = 0; i < this.series[key].length; i++) this.series[key][i].clear();
	...
	_proto.destroy = function destroy(destroyInstance) {
		var _this5 = this;
		if (destroyInstance === void 0) destroyInstance = true;
		EventHandler.flush();
		this._tooltip.dispose();
		this._emit(Events.onDestroyed);
		if (destroyInstance) Object.keys(this).forEach(function(key) {
			try {
				delete _this5[key];
			} catch (e) {}
		});
	...
	_proto.extend = function extend(name, callback) {
		if (typeof this[name] === "function") throw new Error("The method [" + name + "] does already exist, please use another name.");
		Map.prototype[name] = callback;
	};
	_proto._emit = function _emit(eventName, args) {
		for (var event in Events) if (Events[event] === eventName && typeof this.params[event] === "function") this.params[event].apply(this, args);
	};
	_proto._getSelected = function _getSelected(type) {
		var selected = [];
		for (var key in this[type]) if (this[type][key].element.isSelected) selected.push(key);
		return selected;
	};
	_proto._setSelected = function _setSelected(type, keys) {
		var _this6 = this;
		keys.forEach(function(key) {
			if (_this6[type][key]) _this6[type][key].element.select(true);
		});
	};
...
var index = window.jsVectorMap = /* @__PURE__ */ function() {
